...
function App() {
  ...
  const generateLetter = async () => {
    ...
    const result = await callAiApi(prompt);
    if (result) setLetterResult(result);
  ...
  const renderPreview = () => {
    ...
    // Default Empty State
    return (
      <div className="p-8 max-w-prose mx-auto whitespace-pre-wrap leading-relaxed text-slate-700 text-[15px]">
              ...
              {activeTab === 'portfolio' && (
                <div className="animate-in fade-in duration-300">
                  ...
                  <button onClick={generatePortfolio} disabled={loading.portfolio} className="w-full py-3 bg-orange-600 hover:bg-orange-700 disabled:bg-slate-600 text-white font-bold rounded-lg transition-colors flex justify-center items-center gap-2">
                    {loading.portfolio ? "Creating Web Page..." : "✨ Generate Portfolio Website"}
                  </button>
                </div>
              )}
            </div>
